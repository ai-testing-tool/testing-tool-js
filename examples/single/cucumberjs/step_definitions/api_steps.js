...
When(
  ...
  async function (endpoint, docString) {
    const url = `${this.baseUrl}${endpoint}`;
    this.response = await fetch(url, {
      method: 'POST',
...
Then('the response {string} should be {string}', function (field, expected) {
  assert.strictEqual(
    String(this.responseData[field]),
    expected,
...
Then('the response should have an {string} field', function (fieldName) {
  assert.ok(
    this.responseData[fieldName] !== undefined,
...
Then('the response should be an empty object', function () {
  assert.ok(
    typeof this.responseData === 'object' && !Array.isArray(this.responseData),
...
Then(
  'all items should have {string} equal to {int}',
