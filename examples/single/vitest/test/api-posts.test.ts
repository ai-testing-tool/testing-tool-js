...
qaDescribe('Post Validation', () => {
  qaTest('GET all posts - verify 100 posts returned', async ({ qa }) => {
    await qa.issueKeys(['AUTH-105']);
    await qa.step('GET /posts', async () => {
  ...
  qaTest('GET posts by user ID - verify filtered results', async ({ qa }) => {
    await qa.issueKeys(['AUTH-106']);
    await qa.step('GET /posts?userId=1', async () => {
  ...
  qaTest('GET post with comments - verify comment structure', async ({ qa }) => {
    await qa.issueKeys(['AUTH-107']);
    await qa.step('GET /posts/1/comments', async () => {
