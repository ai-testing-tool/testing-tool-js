...
function buildQaMetaFromResult(input) {
    ...
    for (const attachment of input.attachments ?? []) {
        ...
        try {
            ...
            if (message.title)
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-title', body: message.title });
            if (message.comment) {
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-comment', body: message.comment });
            }
            if (message.suite)
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-suite', body: message.suite });
            if (message.fields) {
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-fields', body: message.fields });
            }
            if (message.parameters) {
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-parameters', body: message.parameters });
            }
            if (message.ignore)
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, { type: 'qa-ignore', body: true });
            for (const att of message.attachments ?? []) {
                (0, qa_javascript_commons_1.applyQaAnnotation)(acc, {
                    type: 'qa-attach',
    ...
    return (0, qa_javascript_commons_1.toQaMetaWire)(acc, { framework: 'playwright', reporter: 'qa-playwright' });
}
