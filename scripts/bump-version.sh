...
#   - qa-*/README.md: rewrites pinned install versions (qa-forge-<pkg>@x.y.z),
...
// Package READMEs — pinned install versions (npm install -D qa-forge-cypress@x.y.z ...).
for (const dir of fs.readdirSync('.')) {
  if (!dir.startsWith('qa-')) continue;
  const readme = `${dir}/README.md`;
  if (!fs.existsSync(readme)) continue;
  rewrite(readme, [
    [new RegExp(`(qa-forge-[a-z-]+)@${SEMVER}`, 'g'), `$1@${version}`],
  ]);
}
EOF
