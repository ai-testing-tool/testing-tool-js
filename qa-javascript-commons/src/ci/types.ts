...
export type CiTemplatePartial = Partial<CiTemplateContext> &
  Pick<CiTemplateContext, 'platform' | 'framework' | 'ingestPath' | 'projectKey'>;
...
export class UnsupportedVariantError extends Error {
  ...
  constructor(platform: CiPlatform, framework: CiFramework, ingestPath: CiIngestPath) {
    super(
      `Unsupported CI template variant: ${platform} / ${framework} / ${ingestPath}`,
    );
