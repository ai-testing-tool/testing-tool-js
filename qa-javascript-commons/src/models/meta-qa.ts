...
function resolveType(ann: QaAnnotationLike): string | undefined {
  ...
  if (message.startsWith('QA Step:')) return 'qa-step';
  if (message.startsWith('QA Attach:')) return 'qa-attach';
  if (message.startsWith('QA Ignore')) return 'qa-ignore';
  return undefined;
}
 ...
 */
export function applyQaAnnotation(
  acc: QaMetaAccumulator,
  ann: QaAnnotationLike,
): void {
  ...
  switch (type) {
    ...
    case 'qa-fields': {
      ...
      try {
        const parsed = JSON.parse(stripPrefix(message, 'QA Fields:')) as unknown;
        const record = asStringRecord(parsed);
        if (record) acc.fields = { ...acc.fields, ...record };
      } catch {
        // ignore malformed
      }
      break;
    ...
    case 'qa-parameters': {
      ...
      try {
        const parsed = JSON.parse(stripPrefix(message, 'QA Parameters:')) as unknown;
        const record = asStringRecord(parsed);
        if (record) acc.parameters = { ...acc.parameters, ...record };
      } catch {
        // ignore malformed
      }
      break;
    ...
    case 'qa-step': {
      ...
      if (name) acc.steps.push({ name, status: 'passed' });
      break;
    }
    case 'qa-step-start': {
    ...
    case 'qa-step-end': {
      const name =
        isRecord(ann.body) && typeof ann.body.name === 'string'
          ? ann.body.name
          : stripPrefix(message, 'QA Step End:');
      const status: QaMetaStepWire['status'] =
        isRecord(ann.body) && ann.body.status === 'failed' ? 'failed' : 'passed';
    ...
    case 'qa-step-failed': {
      ...
      else if (name) acc.steps.push({ name, status: 'failed' });
      break;
    }
    case 'qa-attach': {
      if (isRecord(ann.body)) {
        acc.attachments.push({
          ...
          mime_type:
            typeof ann.body.contentType === 'string'
          ...
          content_ref:
            typeof ann.body.content_ref === 'string'
...
export function applyQaAnnotations(
  acc: QaMetaAccumulator,
  annotations: readonly QaAnnotationLike[],
): void {
...
function defaultReporterName(framework: QaMetaFramework): string {
  if (framework === 'jest') return 'qa-jest';
  if (framework === 'mocha') return 'qa-mocha';
  if (framework === 'cucumberjs') return 'qa-cucumberjs';
  if (framework === 'cypress') return 'qa-cypress';
  if (framework === 'playwright') return 'qa-playwright';
  if (framework === 'wdio') return 'qa-wdio';
  return 'qa-vitest';
}
...
export function toQaMetaWire(
  ...
  if (acc.comment !== undefined) wire.comment = acc.comment;
  if (acc.fields && Object.keys(acc.fields).length > 0) wire.fields = acc.fields;
  ...
  if (acc.attachments.length > 0) wire.attachments = acc.attachments;
  if (acc.ignore) wire.ignore = true;

  return wire;
}

/** Build meta.qa from typed helper buffer entries (`{ type, body }`). */
export function qaMetaFromEntries(
  entries: ReadonlyArray<{ type: string; body: unknown }>,
  options: ToQaMetaWireOptions,
): QaMetaWire | undefined {
